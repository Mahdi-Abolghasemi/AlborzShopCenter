...
{
    public class TermAndConditionsService: ITermAndConditionsService
    {
        public IEnumerable<TermAndConditionsDto> Read()
        {
            ...
            path = string.Concat(path, @"\TermAndConditions.txt");

            if (System.IO.File.Exists(path))
            {
                string jsonString = System.IO.File.ReadAllText(path);
                var data = JsonSerializer.Deserialize<List<TermAndConditionsDto>>(jsonString);

                return data;
            }
            else
                return Array.Empty<TermAndConditionsDto>();
        }

        public bool Write(TermAndConditionsDto input)
        {
            string path = System.IO.Directory.GetCurrentDirectory();
            //string contentRootPath = _env.ContentRootPath;
            //string webRootPath = _env.WebRootPath;
            ...
            try
            {
                if(input.Id==Guid.Empty)
                {
                    ...
                    {
                        List<TermAndConditionsDto> data = new List<TermAndConditionsDto>();
                        data.Add(input);
                        jsonString = JsonSerializer.Serialize(data);
                        System.IO.File.WriteAllText(path, jsonString);
                    }
