...
export class Edit_TermAndConditions extends Component {
  constructor(props) {
    super(props);
    this.state = {
      formData: {
      ...
      offset: 0,
      currentPage: 1,
      cardTitle: "New a term and conditions",
    };
  ...
  edit = (item) => {
    this.setState({
      formData: {
        id: item.id,
        editingTime: item.editingTime,
        title: item.title,
        content: item.content,
      },
      showSavePage: 1,
      cardTitle: "Edit term and conditions",
    });
  };
            ...
            {this.sliceData().map((i) => (
              <div class="card mb-2" style={{ width: "100%" }}>
                <div class="card-header table-header">
                  {new Date(i.editingTime).toLocaleString()}
                </div>
                <div class="card-body">
