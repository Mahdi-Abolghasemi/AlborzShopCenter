...
{
    public class BrandRepository<T> : IBrandRepository<T> where T : BrandModel
    {
        ...
        {
            var data = entities.Include(g => g.Group).ThenInclude(c => c.Category).Include(c => c.Country);
        ...
        public IEnumerable<T> Search(SearchBrandDto conditions)
        {
            var data = entities.Include(g => g.Group).ThenInclude(c => c.Category).Include(c => c.Country).Where(item => item.Deleted == false);
            ...
            {
                data = data.Where(item => item.Group.CategoryId == conditions.CategoryId);
            }
